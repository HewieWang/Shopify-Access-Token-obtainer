...
with st.sidebar:
    ...
    # 使用 session_state 或直接由用户输入
    client_id = st.text_input("CLIENT_ID", placeholder="例如: 0c81a35bb3...")
    client_secret = st.text_input("CLIENT_SECRET", type="password", placeholder="例如: shpss_bd28...")
    
    # 可视化配置 Scopes (可以使用多选框)
    available_scopes = [
    ...
    selected_scopes = st.multiselect(
        ...
        default=["read_products", "read_files", "write_files"]
    )
    scopes_str = ",".join(selected_scopes)
